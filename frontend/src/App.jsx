...
export default function App() {
  ...
  useEffect(() => {
    ...
    const executeFetch = () => {
      Promise.all([
        fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8001'}/api/launches${qs}`).then(res => res.json()),
        fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8001'}/api/timeline${qs}`).then(res => res.json()),
        fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8001'}/api/rockets${qs}`).then(res => res.json()),
        fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8001'}/api/success_over_time${qs}`).then(res => res.json()),
        fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8001'}/api/all_data`).then(res => res.json())
